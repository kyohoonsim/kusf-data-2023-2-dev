...
- PlanetScale CLI 설치
    ...
    - homebrew 설치 안 한 경우 homebrew부터 설치할 것.

- pscale 잘 설치되었는지 확인
    - `pscale --version`
    - 다음과 같은 메시지가 나왔다면 잘 설치된 것.
        - pscale version 0.154.0 (build date: 2023-08-28T14:22:41Z commit: 57a8f67)

- pscale 로그인: `pscale auth login`
    - 브라우저에서 Device confirmation 페이지가 열릴텐데, confirm code 버튼 클릭.
    - 그러면 다음과 같이 성공적으로 로그인했다는 메시지가 보일 것.
        - Successfully logged in.

- 아까 만들었던 데이터베이스에 연결: `pscale connect kusf-data`
    - 그러면 다음과 같은 화면이 뜰 것.
    
    ![연결 성공](./images/03_3.png)

#### Windows

- (도프로님이 이 부분은 강의 자료 작성 및 강의도 진행해주시면 좋을 것 같습니다.)

- PlanetScale CLI를 Windows에서 사용하려면 먼저 Go 언어를 설치해야 함.
    - Go는 PlanetScale CLI를 빌드하고 실행하는데 필요한 언어임.
    - Go 다운로드 링크: <https://golang.org/dl/>
    - Go 설치 진행

- 명령 프롬프트(cmd) 실행
    - pscale cli 설치 먕량 실행: `go install github.com/planetscale/cli/pscale@latest`
...
![DBeaver PlanetScale 연결](./images/03_1.png)
